-- ============================================================
-- ERR-08: create_purchase_request fallaba en tiempo de ejecución con
-- "column reference \"raffle_id\" is ambiguous".
--
-- Causa (heredada del original, migración 20260721171031): el UPDATE de
-- expiración usa columnas sin calificar (raffle_id, expires_at) que también
-- son nombres de columnas de salida del RETURNS TABLE, por lo que PL/pgSQL
-- (variable_conflict = error) las considera ambiguas y aborta la función.
--
-- Fix: se califica el UPDATE con un alias de tabla. Se mantiene además la
-- lectura de reservation_minutes (ERR-02). El resto del cuerpo es idéntico.
-- ============================================================
