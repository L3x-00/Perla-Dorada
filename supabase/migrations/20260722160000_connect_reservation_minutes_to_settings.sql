-- ============================================================
-- ERR-02: create_purchase_request debe leer reservation_minutes
-- desde app_settings en lugar del literal '60 minutes'.
--
-- Cambio mínimo: se reemplaza el intervalo fijo por un valor
-- configurable leído de app_settings.reservation_minutes, con
-- respaldo defensivo de 60 minutos si la configuración no existe
-- o es inválida. El resto del cuerpo se mantiene idéntico.
-- ============================================================
...
comment on function public.create_purchase_request(
  uuid,
  text,
  text,
  text,
...
is 'Crea una solicitud pública de manera transaccional. La duración de la reserva se lee de app_settings.reservation_minutes (respaldo 60 min). Solo puede ejecutarse desde el backend con service_role.';
